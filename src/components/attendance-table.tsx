      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-16 text-[var(--color-text-muted)]">
            ...
            <p className="text-sm font-medium text-[var(--color-text-secondary)]">No attendance records yet today</p>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-zinc-100">
              {(showAll ? records : records.slice(0, DEFAULT_VISIBLE)).map((record) => (
                <tr key={record.id} className="text-sm hover:bg-[var(--color-bg-hover)] transition-colors">
                  <td className="px-6 py-3">
                    <div>
                      <p className="font-medium text-[var(--color-text-primary)]">
                        {record.staff?.full_name || 'Unknown'}
                      </p>
                      <p className="text-xs text-[var(--color-text-muted)]">{record.staff?.staff_id}</p>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-3 text-[var(--color-text-secondary)]">
                    {overrideId === record.id ? (
                      <input
                        ...
                        className="text-xs border border-[var(--color-info)]/40 rounded px-1 py-0.5 w-40"
                        disabled={saving}
                      />
                    ) : (
                    ...
                    {overrideId === record.id ? (
                      <div className="flex items-center gap-1">
                        <select
                          ...
                          className="text-xs border border-[var(--color-info)]/40 rounded px-1 py-0.5"
                          disabled={saving}
                        >
                ...
                <tr>
                  <td colSpan={isAdmin ? 6 : 5} className="px-6 py-2">
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-1.5 text-xs font-medium text-[var(--color-info)] hover:text-[var(--color-info)] transition-colors"
                    >
                      {showAll ? (
